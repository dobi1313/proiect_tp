...
void fire_projectile(projectile *projectiles, ship *main_ship) {
    for (int i = 0; i < PROJECTILE_MAX; i++) {
        if (!projectiles[i].is_active) {
            ...
            projectiles[i].x_pos = main_ship->x_pos + main_ship->rect.w / 2 - projectiles->rect.w / 2;
...
void update_position(/*float *x_pos, float *y_pos, int *x_vel, int *y_vel,*/ int up, int down, int left, int right,/* SDL_Rect *dest*/ ship *main_ship) {
